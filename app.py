...
def check_anaphora(text: str, main_entity: str = "") -> dict:
    ...
    # 4. v60: GLOBAL opener frequency — catch scattered keyword stuffing
    # "Jazda po alkoholu stanowi X. (...) Jazda po alkoholu staje się Y."
    # Even if not consecutive, >25% opener ratio = AI pattern
    all_sents = [s.strip() for s in re.split(r'(?<=[.!?])\s+', text) if len(s.strip()) > 10]
    if all_sents and entity_lower:
        opener_count = sum(1 for s in all_sents if s.lower().startswith(entity_lower))
        opener_ratio = opener_count / len(all_sents) if all_sents else 0
        # Max 2 openers per batch (~8-15 sentences) = ~15-25%
        # If >25% OR >3 absolute → flag
        if opener_count > 3 or (len(all_sents) >= 4 and opener_ratio > 0.25):
            total_runs += 1
            if len(examples) < 5:
                examples.append(
                    f"[global-opener] '{entity_lower}' otwiera {opener_count}/{len(all_sents)} "
                    f"zdań ({opener_ratio:.0%}) — max 2 dozwolone"
                )
...
PROBLEMY DO NAPRAWY (WSZYSTKIE 5 WZORCÓW):
...
5. GLOBAL-OPENER: fraza "{main_entity}" otwiera >25% wszystkich zdań w batchu (nawet niekolejnych).
   → MAX 2 zdania w batchu mogą ZACZYNAĆ SIĘ od "{main_entity}".
   → W pozostałych zdaniach przenieś frazę do ŚRODKA lub KOŃCA zdania.
   → ❌ "{main_entity} stanowi wykroczenie." → ✅ "Wykroczenie zachodzi, gdy {main_entity} mieści się w niższym progu."
   → ❌ "{main_entity} skutkuje karą." → ✅ "Sankcje grożące za {main_entity} obejmują grzywnę i areszt."
